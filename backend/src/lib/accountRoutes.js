...
const { listPayments } = require('./courseFees');
...
module.exports = function accountRoutes(role) {
  ...
  // Student-specific administrative reads
  if (role === 'student') {
    ...
    // Payments are handled separately below so that both
    // course-fee receipts and historical payments are included.
    const queries = {
      enrollments: `
        SELECT
          e.*,
    ...
    // Read-only payment history for the selected student.
  ...
  router.post(
    ...
    wrap(async (req, res) => {
      ...
      res.json({
        message:
          'Password reset. Previous sessions are invalidated.'
      });
    })
  );
  ...
  router.delete('/:id', wrap(async (req, res) => {
    ...
    await transaction(async db => {
      ...
      await log(
        ...
        key,
        'DELETE',
        `Deleted ${role}: ${account.full_name}`
      );
    });
